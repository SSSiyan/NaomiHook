...
void DrawEnemyStats() {
    ...
    if (mpLockOnNpc && mpLockOnNpc->mStatus.charaType == eCharaTypeSFF) {
        if (ImGui::TreeNodeEx("mpLockOnNpc HRSFF (Holly Summers)", ImGuiTreeNodeFlags_DrawLinesFull)) {
            ...
            if (hrSff) {
                // uintptr_t baseAddress = reinterpret_cast<uintptr_t>(&hrSff->Padding_0);
                // uintptr_t statusAddress = reinterpret_cast<uintptr_t>(&hrSff->m_ActionTime);
                // uintptr_t offsetDifference = statusAddress - baseAddress;
                // ImGui::Text("Offset target: 0x524 | Offset: 0x%08X", offsetDifference);
