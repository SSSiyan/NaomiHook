        ...
        if (mHRPc* player = nmh_sdk::get_mHRPc()) {
            ...
            if (ImGui::TreeNodeEx("mHRPC tagMAIN", ImGuiTreeNodeFlags_DrawLinesFull)) {
                if (player->tagMain) {
                    ...
                    for (int i = 0; i < 1; ++i) { // I guess 2 can exist but 2nd isn't used? 2nd pointer reads dec 105
                        if (player->tagMain->pNodeMotionBuffer[0]) {
                            // uintptr_t baseAddress = reinterpret_cast<uintptr_t>(&player->tagMain->pGlobalListPrev);
                            // ImGui::Text("Base Address: 0x%08X", baseAddress);
                            // uintptr_t targetAddress = reinterpret_cast<uintptr_t>(&player->tagMain->pNodeMotionBuffer);
                            // ImGui::Text("Target Address: 0x%08X", targetAddress);
                            // uintptr_t offsetDifference = targetAddress - baseAddress;
                            // ImGui::Text("Offset difference: 0x%08X", offsetDifference);
                            ...
                            ImGui::InputFloat3(("Position##" + std::to_string(i)).c_str(), &player->tagMain->pNodeMotionBuffer[i]->Pos.x); // crashing here when I open this header
