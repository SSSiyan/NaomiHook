...
void FreeCam::toggle(bool enable) {
    if (enable) {
        ...
        battle_freecam_patch
            .reset(); //  install_patch_offset(0x3B635F, battle_freecam_patch, "\x74\x07", 2); // je nmh.mHRBattle::mFrameProc+208
        bike_freecam_patch
            .reset(); // install_patch_offset(0x3B55BA, bike_freecam_patch, "\x74\x35", 2); // je nmh.mHRBattle::mCameraFrameProc+D81
    }
}
...
void FreeCam::on_frame() {
    ...
    // --- NEW: F3 = frame advance (when paused) ---
    if (ImGui::IsKeyPressed(ImGuiKey_F3)) {
        // Only step if our pause toggle is currently enabled and a step isn't already active
    ...
    // --- NEW: frame-advance state machine ---
    // Run this before early return so it always completes even if freecam is off this frame.
    if (g_step_pending) {
        // Start step now: unpause for THIS frame
        toggle_pause_enabled = false;
        togglePause(false);
        g_step_unpaused_this_frame = true;
        g_step_pending             = false;
    } else if (g_step_unpaused_this_frame) {
        // We were unpaused last frame; restore pause at the start of THIS frame
